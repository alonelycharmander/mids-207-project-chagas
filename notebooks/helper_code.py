    ...
    for root, directories, files in os.walk(folder):
        for file in files:
            ...
            if extension == file_extension:
                record = os.path.relpath(os.path.join(root, file), folder)[
                    : -len(file_extension)
                ]
                records.add(record)
...
def get_label(string, allow_missing=False):
    ...
    if not has_label and not allow_missing:
        raise Exception(
            "No label is available: are you trying to load the labels from the held-out data?"
        )
...
def get_probability(string, allow_missing=False):
    ...
    if not has_probability and not allow_missing:
        raise Exception(
            "No probability is available: are you trying to load the labels from the held-out data?"
        )
...
def compute_auc(labels, outputs):
    ...
    auprc = sklearn.metrics.average_precision_score(
        labels, outputs, average="macro", pos_label=1, sample_weight=None
    )
...
def is_boolean(x):
    if (is_number(x) and float(x) == 0) or (
        remove_extra_characters(x).casefold() in ("false", "f", "no", "n")
    ):
        return True
    elif (is_number(x) and float(x) == 1) or (
        remove_extra_characters(x).casefold() in ("true", "t", "yes", "y")
    ):
        return True
    else:
        return False
